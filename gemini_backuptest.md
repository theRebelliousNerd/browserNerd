# BrowserNERD: Standalone Build Tool
...
> **REMINDER:** This tool is part of dev_tools - standalone utilities that BUILD SymbioGen. This tool MUST remain self-contained, reusable in other projects, and have ZERO dependencies on SymbioGen runtime code (ai_engine/, api/, common/, workers/). See [../CLAUDE.md](../CLAUDE.md) for full dev_tools philosophy.

## Purpose

Browser automation and intelligence extraction MCP server using Rod (Chrome DevTools Protocol) and Mangle (logic programming). Provides session management, CDP event ingestion, React Fiber reification, and logic-based assertions for browser automation workflows.

## Key Features

- MCP stdio/SSE integration
- Session controls (list, create, attach, fork)
- React component reification
- DOM snapshotting and ingestion
- Fact-based logic programming with Mangle
- CDP event streaming (network, console, navigation, DOM)
...
|-----------|---------|-----------|
| **`mcp-server/`** | Go-based MCP server implementation | [mcp-server/CLAUDE.md](mcp-server/CLAUDE.md) |
| **`docs/`** | Documentation, Mangle references | [docs/mangle-programming-references/CLAUDE.md](docs/mangle-programming-references/CLAUDE.md) |

### MCP Server Packages

| Package | Purpose | CLAUDE.md |
|---------|---------|-----------|
| **`mcp-server/cmd/`** | Server entry point | [mcp-server/cmd/CLAUDE.md](mcp-server/cmd/CLAUDE.md) |
| **`mcp-server/internal/`** | Core implementation | [mcp-server/internal/CLAUDE.md](mcp-server/internal/CLAUDE.md) |
| **`mcp-server/internal/browser/`** | Rod session management, CDP events | [mcp-server/internal/browser/CLAUDE.md](mcp-server/internal/browser/CLAUDE.md) |
| **`mcp-server/internal/mcp/`** | MCP server and tools | [mcp-server/internal/mcp/CLAUDE.md](mcp-server/internal/mcp/CLAUDE.md) |
| **`mcp-server/internal/mangle/`** | Mangle fact engine | [mcp-server/internal/mangle/CLAUDE.md](mcp-server/internal/mangle/CLAUDE.md) |
| **`mcp-server/internal/config/`** | YAML configuration | [mcp-server/internal/config/CLAUDE.md](mcp-server/internal/config/CLAUDE.md) |
| **`mcp-server/schemas/`** | Mangle schema definitions | [mcp-server/schemas/CLAUDE.md](mcp-server/schemas/CLAUDE.md) |
| **`mcp-server/docs/`** | Implementation guides | [mcp-server/docs/CLAUDE.md](mcp-server/docs/CLAUDE.md) |

### Four Core Vectors (from PRD)

| Vector | Purpose | Key Predicates |
|--------|---------|----------------|
| **React Fiber Reification** | Extract component tree | `react_component`, `react_prop`, `react_state` |
| **Flight Recorder** | CDP event stream | `net_request`, `console_event`, `navigation_event` |
| **Session Persistence** | Detached browser state | `current_url`, `dom_node`, `dom_attr` |
| **Logic Assertions** | Causal reasoning | `caused_by`, `slow_api`, `test_passed` |
