"""Condition 5: Playwright MCP runner - same conversation loop, Playwright-based MCP server (Gemini version)."""
...
SYSTEM_PROMPT = """\
You are an information extraction agent using Playwright browser automation tools. \
...
class PlaywrightMCPRunner(BaseRunner):
    ...
    async def setup(self) -> None:
        assert self.settings.playwright_command is not None, (
            "--playwright-command is required for playwright_mcp condition"
        )
