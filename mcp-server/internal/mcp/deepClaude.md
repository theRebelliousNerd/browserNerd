...
| Tool | Description |
|------|-------------|
| `browser-observe` | Safe progressive observation of state, navigation, interactive surfaces, grids, and screenshots |
| `browser-reason` | Mangle-first diagnosis for health, blockers, root causes, and next-best actions |
| `browser-audit` | Phased contract tracing: `discover` persists a deterministic plan/hazards/handles, `execute` replays only explicitly allowed steps with run-state facts, `report` executes correlations, `resume` narrows to selected evidence handles |
