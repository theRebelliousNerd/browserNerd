---
title: **The 'Mangle-MCP' Developer Tooling Architecture Validation: Architecting the Post-Selenium Era of Neuro-Symbolic Agentic Infrastructure**
created: 2025-11-23
...
**First uplift milestone now implemented:** BrowserNERD can recurse from the current browser/frontend context back into a live repository tree without requiring a prebuilt global index. The new repo/code tracing slice uses current URL, React component hints, form affordances, and observed network requests to locate likely frontend request sites, then correlates them with probable backend route, auth, and payload expectations. The result is emitted as structured `repo_trace_*` facts plus evidence handles so MCP/Mangle workflows can consume code-adjacent audit evidence the same way they already consume browser/runtime evidence.

**Wave 3 audit-discovery uplift now implemented:** the browser context capture now stays strictly non-mutating while extracting richer audit inputs from the live page. In addition to form methods/actions/fields, BrowserNERD records navigation link types, write-capable controls, likely destructive controls, and auth-sensitive controls, then derives a deterministic plan skeleton that tells downstream agents what to inspect first without clicking or submitting anything. This closes the gap between "page inventory" and "safe audit plan" while keeping the extraction bounded and reproducible.

**Wave 4 execute-evidence uplift now implemented:** the browser session layer retains a small, session-scoped runtime evidence buffer so execute flows can cheaply tell whether a step changed state. Callers can snapshot evidence after a checkpoint and receive only the latest route change, latest toast, latest console error, and deduplicated request ids observed after that checkpoint. The buffer is intentionally bounded per session to stay lightweight and safe for repeated audit-step execution.

**Wave 5 recursive audit-planning uplift now implemented:** repo/browser discovery now distinguishes internal navigation candidates, control-driven internal navigation, likely write/destructive controls, and same-page reveal surfaces such as disclosures, tabs, controlled panels, and popup openers. This lets BrowserNERD expand safe same-route context first, recurse only into owned routes, and keep risky surfaces in inspect-only or confirm-before-mutation buckets instead of treating every button as an equivalent write hazard.

**Operational contract-audit model now implemented:** BrowserNERD's default progressive-disclosure surface now includes `browser-audit` alongside `browser-observe`, `browser-act`, `browser-reason`, and `browser-mangle`. The audit tool is no longer a one-shot report generator; it is a persisted four-phase workflow. `discover` is passive and emits a deterministic plan plus hazards. `execute` replays the persisted plan with explicit allow gates for risky, navigation, mutating, and destructive actions. `report` performs the full contract synthesis against browser evidence, backend correlations, and recursive repo/code tracing. `resume` re-opens only the requested evidence handles. This workflow is backed by workspace discovery through `.browsernerd/config.yaml` and by a dedicated Mangle model: `audit_plan_state`, `audit_discovered_action`, `scoped_audit_run*`, and `repo_trace_*` facts.

The configuration model is intentionally multi-project. Each target repository can check in its own `.browsernerd/` workspace containing repo-local schema paths, recorder destinations, repo-trace bounds, and Docker container names. Hosts that launch MCP tools from a neutral working directory should pin the intended project with `--workspace-dir`; `--no-workspace` deliberately disables repo-local discovery for shared/global profiles; and `--init-workspace` bootstraps a new checked-in workspace at the current directory or an explicit `--workspace-dir`. Even with workspace discovery, audits still require an explicit `repo_root` so recursive code tracing is always scoped to the intended tree rather than inferred from process cwd.
...
6. React Internals: Virtual DOM vs Fiber Node for Performance Explained, accessed November 22, 2025, [https://dev.to/yorgie7/react-internals-virtual-dom-vs-fiber-node-for-performance-explained-5an5](https://dev.to/yorgie7/react-internals-virtual-dom-vs-fiber-node-for-performance-explained-5an5)
